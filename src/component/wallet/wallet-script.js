...
export const connectWithQRCode = async ({
...
export const connectWithMetamask = async (walletProps) => {
  const { dispatch, walletConnectProvider, supportedChains, proposedChain } =
    walletProps;
  let res;
  ...
  if (!res) {
    ...
    walletProps.proposedChain = proposedChain;
    WS.updateAccount(walletProps);
    // }
...
export const updateAccount = async (walletProps) => {
  ...
  const { ethereum } = window;
  //eth_accounts should not allow metamask to popup on page load //eth_requestAccounts
  const [accounts] = await ethereum.request({
    method: "eth_requestAccounts",
  });
  const networkId = await ethereum.networkVersion;
  await WS.disconnectWalletConnectProvider(walletConnectProvider);
  const getEnv = supportedChains[networkId]
    ? mainnet === supportedChains[networkId].isMainnet
    : false;
  if (!getEnv) {
    WS.disconnectWallet(walletProps);
  ...
  if (!isSupported) {
    ...
    if (!accounts) {
      ...
      dispatch(
        setNotification({
          message:
            "Please connect your site manually from your wallet extension.",
          type: "warning",
        })
      );
    } else {
